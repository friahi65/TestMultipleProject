PACKAGE  ?= github.com/Infoblox-CTO/api-core
...
$(TEST_TARGETS): tests
tests: fmt vendor | $(BASE) ; $(info $(M) running $(NAME:%=% )tests…) @ ## Run tests
...
docker: $(DOCKERFILE) all  ## Build docker image
	...
	@$(DOCKER) build --build-arg binary="$(PACKAGE)" \
					...
					-t $(REGISTRY)/$(PACKAGE):latest .
	#TODO '(PACKAGE):latest' should be reverted to '(VERSION)' once a repo supports versioning.
